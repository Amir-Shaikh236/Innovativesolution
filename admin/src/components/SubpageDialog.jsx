...
/* ═══════════════════════════════════════════════════
   Main component
═══════════════════════════════════════════════════ */
export default function SubpageDialog({ isOpen, onOpenChange, initialData, onSuccess, categories }) {
    ...
    useEffect(() => {
        ...
        if (initialData) {
            ...
            setForm({ ...initialData, category: categoryId, sections: initialData.sections || [] });
            setPreview(
                initialData.image
    ...
    const patchSection = (idx, field, value) =>
        setForm(p => {
            const s = [...p.sections];
                    ...
                    <form id="subpage-form" onSubmit={handleSubmit}>
                        <div className="px-6 sm:px-9 py-7 space-y-8">
                            ...
                            <div className="space-y-5">
                                <SectionDivider title="Page Details" />

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div>
                                        <Label required>Title</Label>
                                        <Input name="title" value={form.title} onChange={set} required placeholder="e.g. Starter Package" className={INPUT} />
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <Textarea name="description" value={form.description} onChange={set} rows={2} placeholder="Brief meta description..." className={TAREA} />
                                </div>

                                <div>
                                    <Label>Main Content</Label>
                                    <Textarea name="content" value={form.content} onChange={set} rows={4} placeholder="Main body text for the page..." className={TAREA} />
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <SectionDivider title="Featured Image" />
                                            ...
                                            <>
                                                ...
                                                <div className="absolute inset-0 bg-black/0 group-hover/preview:bg-black/45 transition-all flex items-center justify-center">
                                                    <button
                                                        ...
                                                        className="opacity-0 group-hover/preview:opacity-100 transition-all flex flex-col items-center gap-1.5 text-white"
                                                    >
                                                ...
                                                <div className="p-5 space-y-4">
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                        ...
                                                        <div>
                                                            ...
                                                            <Input value={section.heading} onChange={e => patchSection(idx, 'heading', e.target.value)} placeholder="Section Heading" required={section.type !== 'cta'} className={INPUT} />
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <Label required>Content Data</Label>
                                                        {section.type === 'cta' ? (
                                                            <Input type="url" value={section.content} onChange={e => patchSection(idx, 'content', e.target.value)} placeholder="https://example.com" required className={INPUT} />
                                                        ) : (
                                                            <Textarea
                                                                rows={section.type === 'packages' ? 6 : 4}
                                                                value={section.content}
                                                                onChange={e => patchSection(idx, 'content', e.target.value)}
                                                                placeholder={section.type === 'packages' ? 'Rich HTML or markdown...' : 'Enter one item per line...'}
                                                                required
                                                                className={TAREA}
                                                            />
                                                        )}
